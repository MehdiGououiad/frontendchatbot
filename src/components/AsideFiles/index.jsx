...
function AsideFiles({ links, showAsideFiles, setShowAsideFiles }) {
  ...
  return (
    <div className={`${showAsideFiles ? "lg:block w-[20%]" : 'flex items-center'}`}>
      {showAsideFiles ? (
        <div
          className={`flex h-full relative `}
          title="Vous retrouverez ici les liens et fichiers envoyés par le chatbot"
        >
          <div className=" flex items-center absolute  top-1/2 transform -translate-y-1/2">
          <img src="toggle-close.png" className="" onClick={()=>setShowAsideFiles(false)} />
          </div>
          <div>
        ...
        <div className="">
          <img src="toggle.svg" onClick={()=>setShowAsideFiles(true)} />
        </div>
      )}
    </div>
  );
